...
public class AdverbiosDAOImpl extends JdbcDaoSupport implements AdverbiosDAO {
	...
	abstract class SeleccionAbstractaSinSignificado extends MappingSqlQuery {
		...
		protected Object mapRow(ResultSet resultset, int j) throws SQLException {
			...
			m1.setGrado(GradoComparacion.getEnum(resultset
					.getString("GRADO_COMPARACION")));
			m1.setTipo(TipoAdverbio.getEnum(resultset
					.getString("TIPO_ADVERBIO")));
			return m1;
		}
	...
	abstract class SeleccionAbstracta extends MappingSqlQuery {
		...
		protected Object mapRow(ResultSet resultset, int j) throws SQLException {
			...
			hashmap.put(Idioma.getEnum(Configuracion.getIdiomaSignificados()),
					q1);
	...
	private void puebla() {
		...
		stringbuffer
				.append("    LEFT JOIN SIGNIFICADOS SIG                 \n");
		stringbuffer
				.append("      ON ADV.ADVERBIO_ID=SIG.REFERENTE_ID       \n");
		stringbuffer.append("WHERE  \n");
		stringbuffer.append("  ADV.LETRA=?    \n");
		stringbuffer.append((new StringBuilder())
				.append("  AND (SIG.IDIOMA IS NULL OR SIG.IDIOMA='")
				.append(Configuracion.getIdiomaSignificados())
				.append("')    \n").toString());
	...
	public List getRegistros(List list) {
		...
		for (Iterator iterator = list1.iterator(); iterator.hasNext(); arraylist
			...
			String s1;
			for (Iterator iterator1 = list2.iterator(); iterator1.hasNext(); stringbuffer
					.append((new StringBuilder()).append("'").append(s1)
							.append("',").toString()))
				s1 = (String) iterator1.next();

			if (stringbuffer.length() > 0)
				stringbuffer.deleteCharAt(stringbuffer.length() - 1);
			String s = SELECCION_POR_IDS_CON_SIGNIFICADO_SQL.replaceFirst(
	...
	public AdverbioBean getInidvidual(String s) {
		List list = seleccionPorIds.execute(new Object[] { s });
		if (list.size() == 0)
			return null;
		else
	...
	public void inserta(AdverbioBean m1) {
		...
		insercion.update(new Object[] { s, m1.getLetra(),
				Integer.valueOf(m1.getCodigo()), m1.getAdverbio(),
				Particularidad.getString(m1.getPartic()),
				GradoComparacion.getLetra(m1.getGrado()),
				TipoAdverbio.getCadena(m1.getTipo()) });
		m1.setId(s);
	}

	public void modifica(AdverbioBean m1) {
		modificacion.update(new Object[] { m1.getLetra(),
				Integer.valueOf(m1.getCodigo()), m1.getAdverbio(),
				Particularidad.getString(m1.getPartic()),
				GradoComparacion.getLetra(m1.getGrado()),
	...
	public void initDao() throws Exception {
		...
		modificaCodigoIndividual = new ModificaCodigoIndividual(
				getDataSource(), MODIFICACION_CODIGO_SQL);
	}
