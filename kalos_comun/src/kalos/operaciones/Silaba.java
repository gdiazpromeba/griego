...
public class Silaba
{
    ...
    private boolean B()
    {
        CarPos a = OpPalabras.encuentraVocalHaciaElComienzo(B, B.length() - 1);
        if(a == null)
            return false;
        CarPos a1 = CarPos.getCarPos(B, a.getPosicion() - 1);
    ...
    private boolean A()
    {
        ...
        if(OpPalabras.esDiptongo(a1.getCaracterGriego(), a.getCaracterGriego()))
            return a.esLarga() || !((a1.getLetraBase() == '\u23B1' || a1.getLetraBase() == '\u03BF') & (a.getLetraBase() == '\u23B9') & (!a1.esLarga()) & (!a.esLarga()));
        else
            return false;
    }
    ...
    public Silaba[] diptongoRompible()
    {
        ...
        if(a.getLetraBase() == '\u23B9' || a.getLetraBase() == '\u23C5')
            return (new Silaba[] {
                new Silaba(getCadena().substring(0, a.getPosicion() + 1)), new Silaba(getCadena().substring(a.getPosicion() + 1))
            });
        else
            return (new Silaba[] {
                this
    ...
    public Object clone()
    {
        return new Silaba(new String(B));
    }
    ...
    public Silaba[] rompeDiptongo()
    {
        ...
        if(OpPalabras.esDiptongo(a1.getCaracterGriego(), a.getCaracterGriego()))
        {
            String s = B.substring(0, a1.getPosicion() + 1);
            String s1 = B.substring(a.getPosicion());
            return (new Silaba[] {
                new Silaba(s), new Silaba(s1)
            });
        } else
        {
            return (new Silaba[] {
