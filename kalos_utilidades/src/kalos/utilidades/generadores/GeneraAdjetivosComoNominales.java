 ...
 * @author GDiaz
 *  
 *Generación dese cero de la tabla ADJETIVOS_COMO_NOMINALES, la cual es muy útil para realizar el 
 *análisis morfológico de los adjetivos utilizando los mismos mecanismos que para los sustantivos.
 *
 *Consta de dos partes principales: una que genera entradas basándose en la tabla de adjetivos misma, y
 *otra que las genera basándose en la tabla IRR_ADJETIVOS_ENTEROS.
 *
 *Debe ejecutarse cada vez que la lista de adjetivos cambie de cualquier manera. Y, por supuesto, cada vez 
 *que las desinencias nominales cambien. 
 *
**/
public class GeneraAdjetivosComoNominales {
	...
	public static void main(String[] args) throws Exception {
		try {
			...
			gerenteIrrAdjetivosEnteros = (GerenteIrrAdjetivosEnteros) contexto
			.getBean("gerenteIrrAdjetivosEnteros");
	...
	/**
	 * Genera entradas de "AdjetivoComoNominal" a partir de la tabla de adjetivos, y las inserta en 
	 * la tabla ADJETIVOS_COMO_NOMINALES.
	 * Cada entrada de "ADJETIVOS" generará una entrada "AdjetivoComoNominal" por género que tenga.
	 * La inserción se hace en lotes (batches) aprovechando que insertaVarios es transaccional. 
	 * @throws Exception
	 */
	private static void generaDesdeAdjetivos() throws Exception{
		...
		for (int i = 0; i < Beta.arrBeta.length; i++) {
			...
			while(todosLetra.size()>0){
				List<AdjetivoBean> lote=new ArrayList<AdjetivoBean>(todosLetra.subList(0, Math.min(segmento, todosLetra.size())));
				...
				for (Iterator<AdjetivoBean> it=lote.iterator(); it.hasNext();) {
					...
					//irregulares
					if (ea.getTipoAdjetivo() == 11 || ea.getTipoAdjetivo()==4) //irregulares (11)  or géneros aislados (4)
						continue;
	...
	/**
	 * Genera entradas AdjetivoComoNominal a partir de la tabla IRR_ADJETIVOS_ENTEROS, y las inserta en 
	 * ADJETIVOS_COMO_NOMINALES. Cada entrada de IRR_ADJETIVOS genera una en ADJETIVOS_COMO_NOMINALES.
	 * La inserción se hace en batches aprovechando que insertaVarios es transaccional. 
	 * @throws Exception
	 */
	...
	/**
	 * Genera valores del tipo "AdjetivoComoNominal" usando la tabla de IRR_ADJETIVOS_ENTEROS.
	 * Con los valores generados incrementa la lista listaACN.
	 * @param ea
	 * @param listaACN
	 * @throws Exception
	 */
	private static void generaEntradaNominalDesdeIrr(AdjetivoBean ea, List<AdjetivoComoNominalBean> listaACN) throws Exception {
